...
- **Verification Flow**: Users must confirm email via a 6-digit code sent by `mailerClient`. Pending registrations expire after 10 minutes.
...
- **Middleware**: `registerLimiter` (5 req/15 min); `validateRegister`
- **Request Body**:
  ```json
  {
...
- **Middleware**: `verifyLimiter` (5 req/15 min); `validateVerify`
- **Request Body**:
  ```json
  {
    "email": "user@example.com",
...
- **Middleware**: `loginLimiter` (5 req/15 min); `validateLogin`
- **Request Body**:
  ```json
  {
...
- **Middleware**: `requireAuth` (ensures valid access token)
...
- **Middleware**: `refreshLimiter` (10 req/15 min); `validateRefresh`
...
- `verifyRegistration` – Validates the submitted code within a Prisma transaction, marks user as verified, issues token pair, and returns sanitized user data.
...
- Verification codes expire after 10 minutes; stale requests are cleaned during verification.
- Rate limits mitigate brute force attacks; tune `max` and `windowMs` as needed.
...
- Move rate-limit windows to configuration for environment-specific tuning.
